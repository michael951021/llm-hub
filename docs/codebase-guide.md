...
- **The macOS keychain guard in the e2e suite (`e2e/smoke.test.ts`, `guardMacKeychainIdentity`).** `config.NewIdentity()` keys its keychain entry off a fixed service/account, not off `MODELHUB_CONFIG_DIR` — correct for production (one real machine, one real identity) but it means a developer running the e2e suite on a Mac that already has a real (or leftover-test) keychain entry will silently reuse it, and the server will reject the "new" node as already enrolled. The guard refuses to proceed and tells you the exact `security delete-generic-password` command, or set `MODELHUB_E2E_CLEAR_KEYCHAIN=1` to have it clear the entry itself. CI never hits this (fresh Linux container, no keychain).
